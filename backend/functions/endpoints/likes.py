...
from firebase import getDB, jsonResponse, getSeverTimeStamp
        ...
        if validateLike(reviewData["movieId"], uid):
            ...
            for doc in query:
                ...
                if reviewData["rating"].lower() == "like":
                    likes = movieData.get("likes", 0) + 1
                    doc.reference.update({"likes": likes,"dislikes": movieData.get("dislikes", 0),"voteCount": likes + movieData.get("dislikes", 0)})
                else:
                    dislikes = movieData.get("dislikes", 0) + 1
                    doc.reference.update({"likes": movieData.get("likes", 0),"dislikes": dislikes,"voteCount": movieData.get("likes", 0) + dislikes})
                break
...
def updateLike(db, reviewData):
    ...
    likeDoc.reference.update({"rating": newRating, "updatedAt": getSeverTimeStamp()})
    ...
    for doc in movieQuery:
        ...
        doc.reference.update({"likes": likes,"dislikes": dislikes, "voteCount": likes + dislikes})
        break
    return
...
def getUserLikes(req):
    ...
    # sorts the likes by most recent to oldest
    likes.sort(key=lambda x: x["updatedAt"], reverse=True)

    return jsonResponse({"ok": True,"data": likes})


def getUserLikeStats(req):
    """
    accesses database for likes collection and then returns all the likes given user has done (likes and dislikes)
